...
function EventList() {
  ...
  useEffect(() => {
    const testConnection = async () => {
      try {
        ...
        setHealthStatus(
          'Connection failed. Check the console for CORS or Network errors.'
        );
        setIsHealthy(false);
      }
    };
  ...
  return (
    <div>
      ...
      <div
        className={`alert ${isHealthy === null ? 'alert-info' : isHealthy ? 'alert-success' : 'alert-danger'} mb-4`}
        role="alert"
      >
        <span
          className={`status-dot ${isHealthy === null ? '' : isHealthy ? 'status-dot--healthy' : 'status-dot--error'}`}
        />
